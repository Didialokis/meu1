...
class ArticleStyleBERTDataset(Dataset):
    def __init__(self, corpus_sents_list, tokenizer_instance: BertTokenizer, seq_len_config: int):
        self.tokenizer = tokenizer_instance; self.seq_len = seq_len_config; self.corpus_sents = [s for s in corpus_sents_list if s and s.strip()]; self.corpus_len = len(self.corpus_sents)
...
class PretrainingTrainer:
    def __init__(self, model: ArticleBERTLMWithHeads, train_dataloader, val_dataloader,
        ...
        self.logger = logging.getLogger(self.__class__.__name__); self.dev = device; self.model = model.to(self.dev)
    ...
    def _save_checkpoint(self, epoch, shard_num, is_best):
        checkpoint = {'epoch': epoch, 'shard_num': shard_num, 'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.opt_schedule._optimizer.state_dict(), 'scheduler_state_dict': self.opt_schedule.state_dict(), 'best_val_loss': self.best_val_loss}
        torch.save(checkpoint, self.checkpoint_path)
        self.logger.info(f"Checkpoint salvo em: {self.checkpoint_path} (Shard {shard_num}, Época {epoch})")
        if is_best:
    ...
    def _load_checkpoint(self):
        ...
        # Se a época salva foi a 0 (de 1), a próxima época é a 1. Se 1 > epochs_per_shard, começamos do próximo shard.
        last_epoch = checkpoint.get('epoch', -1)
        start_shard = checkpoint.get('shard_num', 0)
        # Se o checkpoint foi salvo no final da última época do shard, começamos no próximo shard.
        # Assumindo que epochs_per_shard=1, se a última época salva foi 0, começamos no shard+1.
        start_shard += 1
        self.logger.info(f"Checkpoint carregado. Resumindo a partir do Shard {start_shard + 1}.")
        return start_epoch, start_shard
    def _run_epoch(self, epoch_num, is_training):
        ...
        total_loss_ep, total_mlm_l_ep, total_nsp_l_ep = 0.0, 0.0, 0.0; tot_nsp_ok, tot_nsp_el = 0, 0
        ...
        for i_batch, data in enumerate(data_iter):
            ...
            if (i_batch + 1) % self.log_freq == 0: data_iter.set_postfix({"L":f"{total_loss_ep/(i_batch+1):.3f}", "MLM_L":f"{total_mlm_l_ep/(i_batch+1):.3f}", "NSP_L":f"{total_nsp_l_ep/(i_batch+1):.3f}", "NSP_Acc":f"{tot_nsp_ok/tot_nsp_el*100:.2f}%", "LR":f"{self.opt_schedule._optimizer.param_groups[0]['lr']:.2e}"})
    ...
    def train(self, num_epochs, current_shard_num):
        # A lógica de carregar o checkpoint agora está no loop principal
        for epoch in range(num_epochs):
            ...
            self._save_checkpoint(epoch, current_shard_num, is_best=is_best)
            self.logger.info("-" * 30)
        self.logger.info(f"Treinamento para o Shard {current_shard_num + 1} concluído.")

# --- Funções do Pipeline ---
def load_data_shard(glob_data_path, args, logger, shard_num: int, stage_name: str = "Training"):
    ...
    shard_sents_list = []
    for ex in tqdm(shard_iterator, desc=f"Processando Shard {shard_num + 1} ({stage_name})", total=args.shard_size):
        sent = ex.get("text")
...
def setup_and_train_tokenizer(glob_data_path, args, logger):
    """Prepara e treina o tokenizador usando o PRIMEIRO shard como amostra."""
    ...
    if not (TOKENIZER_ASSETS_DIR / "vocab.txt").exists():
        sentences_for_tokenizer = load_data_shard(glob_data_path, args, logger, shard_num=0, stage_name="Tokenizer")
...
def run_pretraining_on_shards(glob_data_path, args, tokenizer, pad_id, logger):
    ...
    # Instancia o modelo uma vez fora do loop para carregar o checkpoint
    ...
    # Carrega o checkpoint uma vez para saber de onde começar
    trainer_for_checkpoint_load = PretrainingTrainer(model=bertlm_model, train_dataloader=None, val_dataloader=None, d_model_for_optim=args.model_d_model, lr=args.lr_pretrain, betas=(0.9, 0.999), weight_decay=0.01, warmup_steps=args.warmup_steps, device=args.device, model_save_path=Path(args.output_dir) / "best_model.pth", pad_idx_mlm_loss=pad_id, vocab_size=tokenizer.vocab_size, checkpoint_dir=args.checkpoint_dir)
    _, start_shard = trainer_for_checkpoint_load._load_checkpoint()
    ...
    while True:
        ...
        sentences_list = load_data_shard(glob_data_path, args, logger, shard_num)
        ...
        train_dataset = ArticleStyleBERTDataset(train_sents, tokenizer, args.max_len); val_dataset = ArticleStyleBERTDataset(val_sents, tokenizer, args.max_len) if val_sents else None
        train_dl = DataLoader(train_dataset, batch_size=args.batch_size_pretrain, shuffle=True); val_dl = DataLoader(val_dataset, batch_size=args.batch_size_pretrain, shuffle=False) if val_dataset else None
        
        # Recria o trainer para cada shard, mas o estado do modelo/otimizador já foi carregado
        ...
        # Carrega o estado novamente para garantir que o otimizador e o agendador estejam corretos
        trainer._load_checkpoint()

        trainer.train(num_epochs=args.epochs_per_shard, current_shard_num=shard_num)
        shard_num += 1
...
def parse_args():
    ...
    parser.add_argument("--s3_data_path", type=str, required=True, help="Caminho S3 ou LOCAL para o DIRETÓRIO contendo os arquivos batch_*.jsonl.")
...
def main():
    ...
    logger.info("--- Configurações Utilizadas ---"); [logger.info(f"{arg_name}: {value}") for arg_name, value in vars(ARGS).items()]; logger.info("---------------------------------")
    
    base_data_path = ARGS.s3_data_path.rstrip('/')
    glob_data_path = f"{base_data_path}/batch_*.jsonl"
    
    tokenizer, pad_id = setup_and_train_tokenizer(glob_data_path, ARGS, logger)
    run_pretraining_on_shards(glob_data_path, ARGS, tokenizer, pad_id, logger)
