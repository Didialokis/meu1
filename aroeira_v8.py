...
# --- Definições de Classes (Inalteradas) ---
...
# ... (Outras classes do modelo BERT permanecem inalteradas) ...
...
# --- MODIFICAÇÃO (CHECKPOINTING): Adicionar state_dict e load_state_dict ---
...
# --- MODIFICAÇÃO (CHECKPOINTING): Trainer com lógica para salvar e carregar ---
class PretrainingTrainer:
    def __init__(self, model: ArticleBERTLMWithHeads, train_dataloader, val_dataloader,
        ...
        self.best_model_path = Path(model_save_path)
        self.vocab_size = vocab_size
        self.best_val_loss = float('inf')

        # Configurar caminhos de checkpoint
    ...
    def _run_epoch(self, epoch_num, is_training):
        # (Lógica de _run_epoch inalterada, apenas a exibição do TQDM é feita por um único processo)
    ...
    def train(self, num_epochs):
        ...
        self.logger.info(f"Iniciando pré-treinamento de {start_epoch} até {num_epochs} épocas.")
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if self.val_dl:
                with torch.no_grad():
                    val_loss, _, _ = self._run_epoch(epoch, is_training=False)
            
            is_best = val_loss < self.best_val_loss
            if is_best:
                self.best_val_loss = val_loss
                self.logger.info(f"Nova melhor Val Loss: {self.best_val_loss:.4f}.")
            
            self._save_checkpoint(epoch, is_best=is_best)
            self.logger.info("-" * 30)

        self.logger.info(f"Treinamento concluído. Melhor Val Loss: {self.best_val_loss:.4f}")
        self.logger.info(f"Melhor modelo salvo em: {self.best_model_path}")
        self.logger.info(f"Último checkpoint salvo em: {self.checkpoint_path}")


# --- Funções do Pipeline ---
def setup_data_and_train_tokenizer(args, logger):
    logger.info("--- Fase: Preparação de Dados (Streaming S3) e Tokenizador ---")
    if not args.s3_data_path or not (args.s3_data_path.startswith("s3://") or os.path.exists(args.s3_data_path)):
        raise ValueError("Um caminho S3 ou local válido deve ser fornecido via --s3_data_path.")

    logger.info(f"Carregando dados de: {args.s3_data_path}")
    ...
    _all_sents_list = []
    for ex in tqdm(subset_iterator, desc="Processando stream para memória", total=args.streaming_subset_size):
        sent = ex.get("text")
        if isinstance(sent, str) and sent.strip():
            _all_sents_list.append(sent.strip())

    if not _all_sents_list: raise ValueError("Nenhuma sentença foi carregada.")
    logger.info(f"Total de sentenças na memória: {len(_all_sents_list)}")

    temp_file_for_tokenizer = Path(args.output_dir) / "temp_for_tokenizer.txt"
    with open(temp_file_for_tokenizer, "w", encoding="utf-8") as f:
        for s_line in _all_sents_list: f.write(s_line + "\n")
    ...
    if not vocab_file_target.exists():
        ...
        wp_trainer.train(files=[str(temp_file_for_tokenizer)], vocab_size=args.vocab_size, min_frequency=args.min_frequency_tokenizer, special_tokens=["[PAD]", "[CLS]", "[SEP]", "[MASK]", "[UNK]"])
    ...
    return tokenizer, tokenizer.pad_token_id, _all_sents_list

def run_bert_pretraining_nsp_mlm(args, tokenizer, pad_id, sentences_list, logger):
    ...
    trainer = PretrainingTrainer(
        model=bertlm_model, train_dataloader=train_dl, val_dataloader=val_dl,
        d_model_for_optim=args.model_d_model, lr=args.lr_pretrain, betas=(0.9, 0.999), 
        weight_decay=0.01, warmup_steps=args.warmup_steps, device=args.device,
        model_save_path=Path(args.output_dir) / "best_model.pth",
        pad_idx_mlm_loss=pad_id, vocab_size=tokenizer.vocab_size, log_freq=args.logging_steps,
        checkpoint_dir=args.checkpoint_dir
    )
    
    trainer.train(num_epochs=args.epochs_pretrain)

# --- Função Principal e Parseador de Argumentos ---
def parse_args():
    parser = argparse.ArgumentParser(description="Script autônomo de Pré-treino BERT com Checkpointing.")
    ...
    parser.add_argument("--streaming_subset_size", type=int, default=200000, help="Número de exemplos para carregar do stream para a memória.")
    parser.add_argument("--output_dir", type=str, default="./bert_outputs", help="Diretório para salvar outputs, incluindo tokenizador e modelo.")
    ...
    # --- Configurações de Tokenizer e Modelo ---
    parser.add_argument("--max_len", type=int, default=128)
    parser.add_argument("--vocab_size", type=int, default=30522)
    parser.add_argument("--min_frequency_tokenizer", type=int, default=2)
    parser.add_argument("--model_d_model", type=int, default=768) # Dimensão hidden (BERT-base)
    parser.add_argument("--model_n_layers", type=int, default=12)    # N layers (BERT-base)
    parser.add_argument("--model_heads", type=int, default=12)     # N attention heads (BERT-base)
...
def main():
    ...
    log_file = Path(ARGS.output_dir) / 'training_log.log'
